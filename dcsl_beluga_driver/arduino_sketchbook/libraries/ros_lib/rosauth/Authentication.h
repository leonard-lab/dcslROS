...
static const char AUTHENTICATION[] = "rosauth/Authentication";
  ...
  class AuthenticationRequest : public ros::Msg
  {
    ...
    {
      int offset = 0;
      uint32_t * length_mac = (uint32_t *)(outbuffer + offset);
      *length_mac = strlen( (const char*) this->mac);
      offset += 4;
      memcpy(outbuffer + offset, this->mac, *length_mac);
      offset += *length_mac;
      uint32_t * length_client = (uint32_t *)(outbuffer + offset);
      *length_client = strlen( (const char*) this->client);
      offset += 4;
      memcpy(outbuffer + offset, this->client, *length_client);
      offset += *length_client;
      uint32_t * length_dest = (uint32_t *)(outbuffer + offset);
      *length_dest = strlen( (const char*) this->dest);
      offset += 4;
      memcpy(outbuffer + offset, this->dest, *length_dest);
      offset += *length_dest;
      uint32_t * length_rand = (uint32_t *)(outbuffer + offset);
      *length_rand = strlen( (const char*) this->rand);
      offset += 4;
    ...
    {
      int offset = 0;
      uint32_t length_mac = *(uint32_t *)(inbuffer + offset);
      offset += 4;
      ...
      offset += length_mac;
      uint32_t length_client = *(uint32_t *)(inbuffer + offset);
      offset += 4;
      ...
      offset += length_client;
      uint32_t length_dest = *(uint32_t *)(inbuffer + offset);
      offset += 4;
      ...
      offset += length_dest;
      uint32_t length_rand = *(uint32_t *)(inbuffer + offset);
      offset += 4;
