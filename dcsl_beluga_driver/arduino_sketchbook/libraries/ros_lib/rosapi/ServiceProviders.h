  ...
  class ServiceProvidersResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_providersi = (uint32_t *)(outbuffer + offset);
      *length_providersi = strlen( (const char*) this->providers[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < providers_length; i++){
      uint32_t length_st_providers = *(uint32_t *)(inbuffer + offset);
      offset += 4;
