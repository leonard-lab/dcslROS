  ...
  class ServicesResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_servicesi = (uint32_t *)(outbuffer + offset);
      *length_servicesi = strlen( (const char*) this->services[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < services_length; i++){
      uint32_t length_st_services = *(uint32_t *)(inbuffer + offset);
      offset += 4;
