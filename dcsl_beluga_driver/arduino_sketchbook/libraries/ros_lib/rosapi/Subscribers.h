  ...
  class SubscribersResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_subscribersi = (uint32_t *)(outbuffer + offset);
      *length_subscribersi = strlen( (const char*) this->subscribers[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < subscribers_length; i++){
      uint32_t length_st_subscribers = *(uint32_t *)(inbuffer + offset);
      offset += 4;
